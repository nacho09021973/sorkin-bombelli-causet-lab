...
- K1–K6 establish local/geometric Kerr known-truth diagnostics.
- K7 verifies the equatorial null radial potential and circular photon-orbit
  identities.
- K8 integrates the radial ODE for safe control cases.
- K9 integrates the full equatorial RHS `t(lambda)`, `r(lambda)`, `phi(lambda)`.
- K10 audits initial-value geodesic segments and diagnostic `Delta_t` /
  `Delta_phi`.
- K11 performs synthetic known-answer shooting: targets are generated by
  forward integration and then recovered by the shooting routine.

Explicitly not shown:

- no sprinkling event-pair reachability;
- no production Kerr causal classifier;
- no arbitrary point-to-point solver;
- no global Kerr causal relation;
- no Level-B Hawking/Bekenstein discrete rediscovery.

Interpretation guardrails:

- `synthetic_target_hit` is not physical causal reachability.
- S4-THERMO-001 remains a Level-A closed-form horizon/thermo guardrail.
