#!/usr/bin/env python3
"""Exploratory N=24 thermal half-life probe for SORKIN-2.

Scans h where gamma = 2^(-1/h) for one known-truth N=24 case, one
optimizer seed, and one medium budget. This is a thermal schedule
diagnostic only, not confirmation.
"""
...
CSV_PATH = OUT_DIR / "thermal_halflife_probe_n24.csv"
MD_PATH = OUT_DIR / "thermal_halflife_probe_n24.md"
SVG_PATH = OUT_DIR / "thermal_halflife_probe_n24.svg"
...
HALF_LIVES = (1, 2, 3, 4, 5, 6, 8, 10, 12)
...
def write_svg(rows: list[dict[str, object]]) -> None:
    ...
    chunks = [
        ...
        f"  <text x='{width / 2:.0f}' y='30' text-anchor='middle' font-family='serif' font-size='22' fill='#222'>N=24 thermal half-life probe</text>",
    ]
...
def write_markdown(rows: list[dict[str, object]], errors: list[str]) -> None:
    ...
    metrics_agree = (
        int(best_f1["h"]) == int(best_recall["h"]) == int(best_missing["h"])
    )
    sorted_by_f1 = sorted(rows, key=lambda row: float(row["causal_f1"]), reverse=True)
    top_f1 = [row for row in sorted_by_f1 if float(row["causal_f1"]) >= float(best_f1["causal_f1"]) - 0.02]
    if len(top_f1) >= 3:
        window_text = (
            "Within this one-run probe, the top causal-F1 rows within 0.02 of the best "
            f"span h `{min(int(row['h']) for row in top_f1)}` to `{max(int(row['h']) for row in top_f1)}`, "
            "which is more consistent with a broad window than a sharp optimum."
        )
    else:
        window_text = (
            "Within this one-run probe, the near-best causal-F1 set is narrow, but the "
            "single case, single optimizer seed, and single budget are not enough to "
            "separate a sharp optimum from noise."
        )
    organize_text = (
        "This half-life view makes the schedule scale explicit, but with one N=24 case "
        "and one optimizer seed it does not yet resolve the previous gamma ambiguity."
    )
    ...
    lines.extend(
        [
            ...
            "",
            f"- Best causal F1: h `{int(best_f1['h'])}` (gamma `{float(best_f1['gamma']):.10f}`, F1 `{float(best_f1['causal_f1']):.6f}`).",
            f"- Best recall: h `{int(best_recall['h'])}` (gamma `{float(best_recall['gamma']):.10f}`, recall `{float(best_recall['causal_recall']):.6f}`).",
            f"- Fewest missing relations: h `{int(best_missing['h'])}` (gamma `{float(best_missing['gamma']):.10f}`, missing `{int(best_missing['missing_relations_count'])}`).",
            f"- Do metrics agree? {'Yes' if metrics_agree else 'No'} under this restricted probe.",
            f"- Broad window or sharp optimum? {window_text}",
            f"- Does this organize the previous gamma ambiguity better? {organize_text}",
            "",
            "## Guardrails",
            "",
            "- One N=24 case only.",
