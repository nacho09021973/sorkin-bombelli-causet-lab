#!/usr/bin/env python3
"""N=36 optimizer-seed budget/misalignment probe for SORKIN-2.

Exploratory only: fixed family/case/schedule, sweep the canonical
optimizer-seed set at N=36 with short and medium budgets to test
whether the short->medium energy/causal-metric misalignment seen at
optimizer seed 1987 persists across seeds or was seed-specific. This
mirrors the N=30 seed-sweep probe and complements it at the next N.

The optimizer seed 1987 short/medium rows already exist in
``explore/causal_order_budget_scaling_n36/`` and are reused here.
N=36 medium budget is expensive; rows are written one-at-a-time so the
runner is resumable.
"""
...
CSV_PATH = OUT_DIR / "causal_order_budget_scaling_n36_seedsweep.csv"
MD_PATH = OUT_DIR / "causal_order_budget_scaling_n36_seedsweep.md"
SVG_PATH = OUT_DIR / "causal_order_budget_scaling_n36_seedsweep.svg"
LOG_PATH = OUT_DIR / "causal_order_budget_scaling_n36_seedsweep.log"
COMMAND = (
    "python3 explore/causal_order_budget_scaling_n36_seedsweep/"
    "run_causal_order_budget_scaling_n36_seedsweep.py"
)
...
def _load_reuse_rows() -> list[dict[str, object]]:
    """Pull seed=1987 N=36 short/medium rows from the N=36 reuse CSV."""
...
def write_svg(summary: list[dict[str, object]]) -> None:
    ...
    chunks = [
        ...
        "font-family='serif' font-size='22' fill='#222'>N=36 optimizer-seed "
...
def write_markdown(
    ...
    lines = [
        ...
        "",
        "At N=36, fixed family/case/schedule/budgets, does the short -> medium "
    ...
    lines += [
        ...
        "",
        f"- Does the N=36 short->medium misalignment persist across "
        ...
        "- Is the N=30 seed-specific result repeated or overturned at N=36? "
        f"`{'overturned (N=36 misalignment persists across optimizer seeds while N=30 was seed-specific) -- suggests genuine N-dependent regime change' if outcome == 'misalignment_persists_across_optimizer_seeds' else 'repeated (N=36 misalignment also looks seed-specific, mirroring the N=30 finding) -- weakens any N-driven transition story' if outcome == 'misalignment_appears_seed_specific' else 'mixed; pattern at N=36 is not seed-uniform and does not cleanly mirror or overturn N=30'}`.",
        "- Does this justify a topology / case-seed follow-up at N=36? "
        f"`{'yes; if optimizer-seed misalignment is robust, a multi-case-seed probe at N=36 short and medium would test topology specificity next' if outcome == 'misalignment_persists_across_optimizer_seeds' else 'low priority for topology probe; the multi-seed pattern aligns with N=30 and suggests basin-selection drives the seed-1987 misalignment rather than topology' if outcome == 'misalignment_appears_seed_specific' else 'yes; a mixed outcome warrants broader optimizer-seed and case-seed exploration before any claim'}`.",
...
def main() -> int:
    ...
    start = time.perf_counter()
    _log("starting N=36 optimizer-seed budget/misalignment probe")
    ...
    _log("finished N=36 optimizer-seed budget/misalignment probe")
    return 0
