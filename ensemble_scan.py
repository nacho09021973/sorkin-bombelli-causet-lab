#!/usr/bin/env python3
"""Parallel ensemble exploration for cones.py.

This is the step beyond a single schedule sweep:
- run many seeds in parallel
- aggregate success rates and energy statistics
- visualize phase-like behavior over the schedule grid
"""
...
def _run_job(
    args: Tuple[List[List[bool]], int, int, float, float, int, int, int, str],
    shared_cuda_backend: Any | None = None,
) -> RunRow:
    z, dim, seed, initial_temp, cooling_factor, warmup_limit, anneal_limit, max_data, backend = args
    with tempfile.TemporaryDirectory(prefix="ensemble_scan_") as tmpdir:
        out_path = Path(tmpdir) / f"seed{seed}.out"
        sim = ConesSimulator(
            z=z,
            dim=dim,
            seed=seed,
...
def summarize(rows: Sequence[RunRow], zero_threshold: float) -> List[SummaryRow]:
    ...
    for (initial_temp, cooling_factor), items in sorted(grouped.items()):
        ...
        success_rate = sum(1 for value in finals if value <= zero_threshold) / len(finals)
...
def write_heatmap(summary: Sequence[SummaryRow], path: Path) -> None:
    ...
    low = min(row.success_rate for row in summary)
    high = max(row.success_rate for row in summary)
    ...
    for yi, temp in enumerate(temps):
        ...
        for xi, factor in enumerate(factors):
            ...
            fill = color_for(row.success_rate, low, high)
            cells.append(
                f"<rect x='{x}' y='{y}' width='{cell_w - 8}' height='{cell_h - 8}' rx='8' fill='{fill}' />"
            )
            cells.append(
                f"<text x='{x + (cell_w - 8)/2:.1f}' y='{y + 28}' text-anchor='middle' font-family='monospace' font-size='12' fill='#111'>{row.success_rate:.2f}</text>"
            )
            cells.append(
                f"<text x='{x + (cell_w - 8)/2:.1f}' y='{y + 48}' text-anchor='middle' font-family='monospace' font-size='11' fill='#111'>m={row.mean_final:.3f}</text>"
            )
  ...
  <text x='{width/2:.0f}' y='32' text-anchor='middle' font-family='serif' font-size='22' fill='#222'>Ensemble success rate</text>
  <text x='{width/2:.0f}' y='{height - 18}' text-anchor='middle' font-family='monospace' font-size='16' fill='#222'>cooling factor</text>
  {''.join(factor_labels)}
  {''.join(cells)}
  <text x='{width - 230}' y='40' font-family='monospace' font-size='12' fill='#444'>green = higher success rate</text>
</svg>
"""
    path.write_text(svg, encoding="utf-8")


def write_markdown(input_file: Path, rows: Sequence[RunRow], summary: Sequence[SummaryRow], heatmap: Path, path: Path, zero_threshold: float) -> None:
    lines = ["# Ensemble Scan", ""]
    ...
    best = max(summary, key=lambda row: row.success_rate)
    lines.append("## Summary")
    lines.append("")
    lines.append(
        f"- Best schedule by success rate: T0={best.initial_temp:.3f}, cooling={best.cooling_factor:.6f}, success rate={best.success_rate:.2f}, mean final={best.mean_final:.6f}"
    )
    lines.append("")
    lines.append("## Per-schedule summary")
    lines.append("")
    lines.append("| T0 | cooling | runs | success_rate | mean_final | median_final | min_final | max_final | best_seed |")
    lines.append("| ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: |")
    for row in sorted(summary, key=lambda r: (-r.success_rate, r.mean_final, r.initial_temp, r.cooling_factor)):
        lines.append(
            f"| {row.initial_temp:.3f} | {row.cooling_factor:.6f} | {row.runs} | {row.success_rate:.2f} | {row.mean_final:.6f} | {row.median_final:.6f} | {row.min_final:.6f} | {row.max_final:.6f} | {row.best_seed} |"
        )
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Parallel ensemble scan for cones.py.")
    parser.add_argument("input_file", type=Path)
    parser.add_argument("--dim", type=int, required=True)
    parser.add_argument("--gpu-first", action="store_true", help="prefer a shared CUDA backend when available")
    parser.add_argument("--seed-start", type=int, default=1959)
    parser.add_argument("--seed-count", type=int, default=12)
    parser.add_argument("--seed-step", type=int, default=1)
    parser.add_argument("--temp-min", type=float, default=100.0)
    parser.add_argument("--temp-max", type=float, default=180.0)
    parser.add_argument("--temp-count", type=int, default=4)
    parser.add_argument("--cooling-min", type=float, default=0.80)
    parser.add_argument("--cooling-max", type=float, default=0.90)
    parser.add_argument("--cooling-count", type=int, default=3)
    ...
    parser.add_argument("--run-csv", type=Path, default=Path("ensemble_scan_runs.csv"))
    parser.add_argument("--summary-csv", type=Path, default=Path("ensemble_scan_summary.csv"))
    parser.add_argument("--report-md", type=Path, default=Path("ensemble_scan.md"))
    parser.add_argument("--heatmap-svg", type=Path, default=Path("ensemble_scan.svg"))
    return parser
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    jobs = [
        (z, args.dim, seed, temp, factor, args.warmup_limit, args.anneal_limit, args.max_data, args.backend)
        for temp in temps
    ...
    try:
        if args.gpu_first and args.workers <= 1 and args.backend in ("cuda", "auto") and CUDAEnergyBackend is not None and cuda_available():
            shared_cuda_backend = CUDAEnergyBackend(len(z), args.dim)
            shared_cuda_backend.set_z(z)

        if args.workers <= 1:
            for job in jobs:
                rows.append(_run_job(job, shared_cuda_backend=shared_cuda_backend))
        else:
            with ProcessPoolExecutor(max_workers=args.workers) as executor:
                for row in executor.map(partial(_run_job, shared_cuda_backend=None), jobs):
                    rows.append(row)
    finally:
