"""Integrity tests for the Phase 1D structural atlas."""
...
ATLAS_CSV = ROOT / "benchmarks" / "foundation" / "phase1d_structural_atlas.csv"
ATLAS_MD = ROOT / "benchmarks" / "foundation" / "phase1d_structural_atlas.md"
...
class Phase1DStructuralAtlasIntegrityTests(unittest.TestCase):
    def setUp(self) -> None:
        self.assertTrue(
            ATLAS_CSV.exists(),
            msg=(
                f"missing atlas CSV at {ATLAS_CSV}; "
                "run `make regen-phase1d`"
            ),
        )
    ...
    def test_covers_full_grid(self) -> None:
        minkowski_keys = {
            (r["target_dim"], int(r["n"]), int(r["seed"]))
    ...
    def test_no_nan_or_inf_in_numeric_columns(self) -> None:
        for row in self.rows:
            for col in NUMERIC_COLUMNS:
                ...
                self.assertNotIn(
                    ...
                    msg=(
                        f"{col}={raw} for cell "
                        f"family={row['family']}, d={row['target_dim']}, "
    ...
    def test_chain_abundance_is_probability(self) -> None:
        for row in self.rows:
            abundance = float(row["chain3_abundance"])
