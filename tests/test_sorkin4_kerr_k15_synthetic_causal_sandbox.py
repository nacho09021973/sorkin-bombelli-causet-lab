"""Regression tests for S4-KERR-K15-SYNTHETIC-CAUSAL-SANDBOX-001."""
...
SCRIPT_PATH = KERR_DIR / "audit_kerr_k15_synthetic_causal_sandbox_001.py"
PREFIX = "kerr_k15_synthetic_causal_sandbox_001_n12_seed1959"
K14_JSON = KERR_DIR / "kerr_k14_multi_winding_synthetic_shooting_001_n12_seed1959.json"
...
class KerrK15SyntheticCausalSandboxTests(unittest.TestCase):
    @classmethod
    ...
    def test_summary_counts_positive(self):
        s = _load_json()["global_summary"]
        assert s["total_pairs"] > 0
        assert s["synthetic_null_connected_pairs"] > 0
        assert s["synthetic_no_match_pairs"] > 0
        assert s["recovered_known_answer_pairs"] > 0
        assert s["recovered_whirling_pairs"] > 0
        assert s["negative_controls_passed"] > 0

    def test_whirling_delta_and_recovery_flags(self):
        s = _load_json()["global_summary"]
        assert s["max_abs_delta_phi_target"] > math.pi
        assert s["any_pi_whirling_pair_recovered"] is True

    def test_conditional_2pi_recovery(self):
        k14 = json.loads(K14_JSON.read_text(encoding="utf-8"))
        has_k14_2pi = any(
            c["source_was_k13b_whirling"]
            and c["physical_whirling_synthetic_target_recovered"]
            and c["abs_delta_phi_target"] > 2.0 * math.pi
            for c in k14["cases"]
        )
        if has_k14_2pi:
            assert _load_json()["global_summary"]["any_2pi_whirling_pair_recovered"] is True

    def test_null_connected_rows_pass(self):
        for c in _load_json()["cases"]:
            if (not c["advisory_only"]) and (not c["unresolved"]) and c["synthetic_pair_classification"] == "synthetic_null_connected":
                assert c["synthetic_pair_recovered"] is True, c["case_id"]
    ...
    def test_negative_control_rows(self):
        for c in _load_json()["cases"]:
            if c["synthetic_pair_classification"] == "synthetic_no_match":
                assert c["synthetic_pair_recovered"] is False, c["case_id"]
    ...
    def test_md_caveats(self):
        ...
        required = [
            "synthetic known-answer event-pair sandbox",
